...
FPTree::FPTree(const std::vector<Transaction>& transactions, int minimum_support_threshold) :
    root( std::make_shared<FPNode>( int{}, nullptr ) ), header_table(),
    minimum_support_threshold( minimum_support_threshold )
{
    // scan the transactions counting the frequency of each item
    std::map<int, int> frequency_by_item;
    ...
    std::set<std::pair<int, int>, frequency_comparator> items_ordered_by_frequency(frequency_by_item.cbegin(), frequency_by_item.cend());
...
std::set<Pattern> fptree_growth(const FPTree& fptree)
{
    ...
    if ( contains_single_path( fptree ) ) {
        ...
        while ( curr_fpnode ) {
            const int& curr_fpnode_item = curr_fpnode->item;
